...
/// secgap
/// Print out the removed memory-assess instruction and related information.
static void printRemoval(Instruction *Instr) {
  DebugLoc DLoc = Instr->getDebugLoc();

  errs() << "--------- possible scrubbing removal ---------\n";
  // Print out the location information.
  errs() << "Location: ";
  DLoc.printSimple(errs());
  errs() << "\n";

  // Print out the instruction.
  errs() << "Removed IR Instruction: ";
  Instr->print(errs());
  errs() << "\n";

  // Print out addintional information.
  errs() << "Additional Info: ";
  DLoc.print(errs());
  errs() << "\n";
}

/// secgap
/// Check whether a store or a call to memset is a scrubbing operation
/// If it is, return the number of bits it attempts to scrub;
/// Otherwise, return 0.
...
/// secgap
/// Get the number of bytes a store instruction attempts to write
static uint64_t getStoreLen(Instruction *I) {
  if (isa<StoreInst>(I)) {
	StoreInst *SI = dyn_cast<StoreInst>(I);
...
/// secgap
/// Get the length operand of a memory-access intrinsic attempts to write
static Value * getInstrinsicLen(Instruction *I) {
  if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
...
/// secgap
/// This function should be used when the number of bytes of a dead store is a constant.
/// NumScrubbed: num of bytes to be scrubbed
/// CountAll: indicates what we are reporting
/// If CountAll is true, then we are reporting the number of bytes of store that the orignal DSE will remove;
/// If CountAll is false, then we are reporting the number of bytes of store that sec-dse will keep.
static void printByteCount(uint64_t NumScrubbed, Instruction *Inst, bool CountAll = false) {

  // Set up printf
  Module *M = Inst->getParent()->getParent()->getParent();
  std::vector<llvm::Type *> args;
  args.push_back(llvm::Type::getInt8PtrTy(M->getContext()));
  // accepts a char*, is vararg, and returns int
  llvm::FunctionType *printfType =
      llvm::FunctionType::get(Type::getInt32Ty(M->getContext()), args, true);
  Constant *printfFunc =
  ...
  Value *formatStr = CountAll ? builder.CreateGlobalStringPtr("\n[dse] remove %llu bytes of store\n"): \
		  	  	  	  	  	  	  builder.CreateGlobalStringPtr("\n[secdse] keep %llu bytes of store\n");
  values.push_back(formatStr);
  Value *NumScrubbedConstant = ConstantInt::get(Type::getInt64Ty(M->getContext()), NumScrubbed);
  values.push_back(NumScrubbedConstant);
  builder.CreateCall(printfFunc, values);
}


static bool printOrNot(Instruction *Inst) {
...
/// secgap
/// This function should be used when the number of bytes of a dead store is not a constant.
/// Since sec-dse never keeps any variable-length stores, this function is only used for reporting
/// the number of bytes of store the normal dse removes.
static void printByteCount(Instruction *Inst) {
  // Set up printf
  Module *M = Inst->getParent()->getParent()->getParent();
  std::vector<llvm::Type *> args;
  args.push_back(llvm::Type::getInt8PtrTy(M->getContext()));
  ...
  if (isa<StoreInst>(Inst)) {
	uint64_t writeCount = getStoreLen(Inst);
	Value *NumScrubbedConstant = ConstantInt::get(Type::getInt64Ty(M->getContext()), writeCount);
	values.push_back(NumScrubbedConstant);
  }
  else {
	values.push_back(getInstrinsicLen(Inst));
  }
...
static bool handleFree(CallInst *F, AliasAnalysis *AA,
  ...
  while (!Blocks.empty()) {
    ...
    while (Dep.isDef() || Dep.isClobber()) {
      ...
      // secgap
      // Now DSE has decided to remove the instruction.
      // First check if this may be a scrubbing operation
      if (Func->hasFnAttribute(Attribute::BCounter)) {
        uint64_t ToBeWritten = isScrubbing(Dependency);
        if (ToBeWritten > 0) {
	      printByteCount(ToBeWritten, Dependency);
        }
      }
      // If sec-dse is enabled, and the instruction to be removed may
      // be a scrubbing instruction based on our heuristic, break from the inner while loop.
      if (Func->hasFnAttribute(Attribute::SecDSE)) {
    	uint64_t ToBeWritten = isScrubbing(Dependency);
	    if (ToBeWritten > 0)
	      break;
      }
      // If we want to count how many bytes of store the original dse removes
      if (Func->hasFnAttribute(Attribute::BCounterAll)) {
        if (printOrNot(Dependency))
          printByteCount(Dependency);
      }
  ...
  for (BasicBlock::iterator BBI = BB.end(); BBI != BB.begin(); ){
    ...
    if (hasMemoryWrite(&*BBI, *TLI) && isRemovable(&*BBI)) {
      ...
      if (AllDead) {
        ...
        // secgap
        // Now DSE has decided to remove the instruction.
        // First check if this may be a scrubbing operation based on our heuristic
        if (Func->hasFnAttribute(Attribute::BCounter)) {
          uint64_t ToBeWritten = isScrubbing(Dead);
          if (ToBeWritten > 0)
        	printByteCount(ToBeWritten, Dead);
        }
        // Second, check if we want to keep this dead store
        if (Func->hasFnAttribute(Attribute::SecDSE)) {
          uint64_t ToBeWritten = isScrubbing(Dead);
  	      if (ToBeWritten > 0)
  	        continue;
        }
        // Third, check if we want to report every dead store that the original dse removes
        if (Func->hasFnAttribute(Attribute::BCounterAll)) {
          if (printOrNot(Dead))
            printByteCount(Dead);
        }
...
static bool eliminateNoopStore(Instruction *Inst, BasicBlock::iterator &BBI,
  ...
  if (LoadInst *DepLoad = dyn_cast<LoadInst>(SI->getValueOperand())) {
    if (SI->getPointerOperand() == DepLoad->getPointerOperand() &&
      ...
      // secgap
      if (Inst->getFunction()->hasFnAttribute(Attribute::BCounterAll)) {
        if (printOrNot(Inst))
          printByteCount(Inst);
      }
    ...
    while (InstDep.isDef() || InstDep.isClobber()) {
      ...
      if (isRemovable(DepWrite) &&
        ...
        if (OR == OverwriteComplete) {
          ...
          // secgap
          // if we want to count the number of bytes of store sec-dse keeps
          if (Func->hasFnAttribute(Attribute::BCounter)) {
            if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(Inst)) {
              if (II->getIntrinsicID() == Intrinsic::lifetime_end) {
            	  uint64_t ToBeWritten = isScrubbing(DepWrite);
            	  if (ToBeWritten > 0)
            		  printByteCount(ToBeWritten, DepWrite);
              }
            }
          }
          // if sec-dse is enabled
          if (Func->hasFnAttribute(Attribute::SecDSE)) {
            if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(Inst)) {
              if (II->getIntrinsicID() == Intrinsic::lifetime_end) {
                uint64_t ToBeWritten = isScrubbing(DepWrite);
          ...
          // if we want to report the number of bytes the orginal dse removes
          if (Func->hasFnAttribute(Attribute::BCounterAll)) {
            if (printOrNot(DepWrite))
              printByteCount(DepWrite);
          }
          ...
          if ((llvm::isPowerOf2_64(InstWriteOffset) &&
        	...
        	// secgap
            // if we want to count the number of bytes sec-dse keeps
        	int64_t EarlierOffset = DepWriteOffset;
        	int64_t LaterOffset = InstWriteOffset;
        	int64_t EarlierSize = (int64_t) DepLoc.Size;
        	int64_t LaterSize = (int64_t) Loc.Size;
        	// Calculate how many bytes are overlapped
        	unsigned ToBeWritten = IsOverwriteEnd ? (EarlierOffset + EarlierSize - LaterOffset + 1) : \
        											(LaterOffset + LaterSize - EarlierOffset + 1);
            if (Func->hasFnAttribute(Attribute::BCounter)) {
              if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(Inst)) {
                if (II->getIntrinsicID() == Intrinsic::lifetime_end) {
                	if (isScrubbing(DepWrite) > 0)
                		printByteCount(ToBeWritten, DepWrite);
                }
              }
            }
            ...
            if (Func->hasFnAttribute(Attribute::BCounterAll)) {
              if (printOrNot(DepWrite))
                printByteCount(ToBeWritten, DepWrite, true);
            }
...
public:
  ...
  bool runOnFunction(Function &F) override {
    ...
    // secgap
    if (F.hasFnAttribute(Attribute::NoDSE))
      return false;
