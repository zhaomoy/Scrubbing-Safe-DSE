...
llvm::Function *CodeGenModule::CreateGlobalInitOrDestructFunction(
  ...
  // secgap
  if (getLangOpts().Sanitize.has(SanitizerKind::SecDSE))
    Fn->addFnAttr(llvm::Attribute::SecDSE);
  if (getLangOpts().Sanitize.has(SanitizerKind::NoDSE))
    Fn->addFnAttr(llvm::Attribute::NoDSE);
  if (getLangOpts().Sanitize.has(SanitizerKind::BCounter))
    Fn->addFnAttr(llvm::Attribute::BCounter);
  if (getLangOpts().Sanitize.has(SanitizerKind::BCounterAll))
    Fn->addFnAttr(llvm::Attribute::BCounterAll);
