...
define void @test1(i32* %Q, i32* %P) secdse {
        %DEAD = load i32, i32* %Q
...
define void @test2(i32 *%p, i32 *%q) secdse {
...
define i32 @test3(i32* %g_addr) nounwind secdse {
; CHECK-LABEL: @test3(
...
define void @test4(i32* %Q) secdse {
        %a = load i32, i32* %Q
...
define void @test5(i32* %Q) secdse {
...
; alias).
define void @test6(i32 *%p, i8 *%q) secdse {
...
; alias).
define void @test7(i32 *%p, i8 *%q, i8* noalias %r) secdse {
  store i32 10, i32* %p, align 4       ;; dead.
...
define void @test9(%struct.x* byval  %a) nounwind secdse {
	%tmp2 = getelementptr %struct.x, %struct.x* %a, i32 0, i32 0
	store i32 1, i32* %tmp2, align 4 ;; not dead
	ret void
; CHECK-LABEL: @test9(
; CHECK: store
}
...
define void @test9_2(%struct.x* inalloca  %a) nounwind secdse {
	%tmp2 = getelementptr %struct.x, %struct.x* %a, i32 0, i32 0
...
define double @test10(i8* %X) secdse {
        %X_addr = alloca i8*
...
define void @test12({ i32, i32 }* %x) nounwind secdse {
...
define i32 addrspace(1)* @test13_addrspacecast() secdse {
  %p = tail call i8* @malloc(i32 4)
...
define void @test14(i32* %Q) secdse {
        %P = alloca i32
        %DEAD = load i32, i32* %Q
        store i32 %DEAD, i32* %P
        ret void
...
define void @test15(i8* %P, i8* %Q) nounwind ssp secdse {
...
define void @test16(i8* %P, i8* %Q) nounwind ssp secdse {
...
define void @test17(i8* %P, i8* noalias %Q) nounwind ssp secdse {
...
define void @test17v(i8* %P, i8* %Q) nounwind ssp secdse {
...
; A = B
; A = A
define void @test18(i8* %P, i8* %Q, i8* %R) nounwind ssp secdse {
  tail call void @llvm.memcpy.p0i8.p0i8.i64(i8* %P, i8* %Q, i64 12, i32 1, i1 false)
...
define void @test19({i32} * nocapture byval align 4 %arg5) nounwind ssp secdse {
bb:
...
; CHECK-LABEL: @test21(
define void @test21() secdse {
  %m = call i8* @calloc(i32 9, i32 7)
  store i8 0, i8* %m ;; dead because of calloc
; CHECK-NEXT: ret void
  ret void
}

; CHECK-LABEL: @test22(
define void @test22(i1 %i, i32 %k, i32 %m) nounwind secdse {
  %k.addr = alloca i32
  %m.addr = alloca i32
...
define noalias i8* @test23() nounwind uwtable ssp secdse {
...
; CHECK: store i32 %c
; CHECK: ret void
define void @test24([2 x i32]* %a, i32 %b, i32 %c) nounwind secdse {
...
; CHECK: store i8 0
; CHECK: store i8 %tmp
define i8* @test25(i8* %p) nounwind secdse {
...
; CHECK-NOT: store
; CHECK: ret
define i32 @test26(i1 %c, i32* %p) secdse {
entry:
...
; CHECK-NOT: store
; CHECK: ret
define i32 @test27(i1 %c, i32* %p) secdse {
entry:
  %v = load i32, i32* %p, align 4
  br i1 %c, label %bb1, label %bb2
bb1:
...
define i32 @test28(i1 %c, i32* %p, i32* %p2, i32 %i) secdse {
entry:
  %v = load i32, i32* %p, align 4
...
; CHECK: bb3:
; CHECK-NEXT: store i32 %v
define i32 @test29(i1 %c, i32* %p, i32* %p2, i32 %i) secdse {
entry:
  %v = load i32, i32* %p, align 4
  br i1 %c, label %bb1, label %bb2
bb1:
...
; CHECK: bb3:
; CHECK-NEXT: store i32 %v
define i32 @test30(i1 %c, i32* %p, i32 %i) secdse {
entry:
  %v = load i32, i32* %p, align 4
  br i1 %c, label %bb1, label %bb2
bb1:
...
; CHECK-NOT: store
; CHECK: ret
define i32 @test31(i1 %c, i32* %p, i32 %i) secdse {
entry:
  %v = load i32, i32* %p, align 4
  br label %bb1
bb1:
...
define i32 @test32(i1 %c, i32* %p, i32 %i) secdse {
entry:
  %v = load i32, i32* %p, align 4
  br label %bb1
bb1:
...
; Remove redundant store, which is in the lame loop as the load.s
; CHECK-LABEL: @test33(
; CHECK-NOT: store
; CHECK: ret
define i32 @test33(i1 %c, i32* %p, i32 %i) secdse {
entry:
  br label %bb1
bb1:
  %v = load i32, i32* %p, align 4
